...
cp:
	ssh -t $(REMOTE) "cd $(REMOTE_PATH) && \
		rm -r ./dist && \
		mkdir -p ./dist && \
		docker cp sdcpp-inspect:/opt/sdcpp/build/bin/sd ./dist/sd && \
		docker cp sdcpp-inspect:/opt/sdcpp/build/bin/sd-server ./dist/sd-server"
	rm -r ./dist && mkdir -p ./dist && \
	rsync -a $(REMOTE):$(REMOTE_PATH)/dist/ ./dist/ && \
	cp ./dist/sd ../lzc-aipod-imagen/models/sd && \
	cp ./dist/sd-server ../lzc-aipod-imagen/models/sd-server
