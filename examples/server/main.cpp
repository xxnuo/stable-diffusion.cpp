...
void parse_args(int argc, const char** argv, SDParams& params) {
    ...
    options.string_options = {
        ...
        {"", "--llm", "path to the llm text encoder. For example: (qwenvl2.5 for qwen-image, mistral-small3.2 for flux2, ...)", &params.ctxParams.llm_path},
        {"", "--llm_vision", "path to the llm vit", &params.ctxParams.llm_vision_path},
        {"", "--qwen2vl", "alias of --llm. Deprecated.", &params.ctxParams.qwen2vl_path},
        {"", "--qwen2vl_vision", "alias of --llm_vision. Deprecated.", &params.ctxParams.qwen2vl_vision_path},
    ...
    options.int_options = {
        ...
        {"", "--chroma-t5-mask-pad", "t5 mask pad size of chroma", &params.ctxParams.chroma_t5_mask_pad},
        {"", "--timestep-shift", "shift timestep for NitroFusion models (default: 0). recommended N for NitroSD-Realism around 250 and 500 for NitroSD-Vibrant", &params.lastRequest.sample_params.shifted_timestep},
        {"", "--preview-interval", "interval in denoising steps between consecutive updates of the image preview file (default is 1, meaning updating at every step)", &params.lastRequest.preview_interval},
    ...
    options.float_options = {
        ...
        {"", "--img-cfg-scale", "image guidance scale for inpaint or instruct-pix2pix models: (default: same as --cfg-scale)", &params.lastRequest.sample_params.guidance.img_cfg},
        ...
        {"", "--slg-scale", "skip layer guidance (SLG) scale, only for DiT models: (default: 0). 0 means disabled, a value of 2.5 is nice for sd3.5 medium", &params.lastRequest.sample_params.guidance.slg.scale},
        ...
        {"", "--eta", "eta in DDIM, only for DDIM and TCD (default: 0)", &params.lastRequest.sample_params.eta},
        ...
        {"", "--flow-shift", "shift value for Flow models like SD3.x or WAN (default: auto)", &params.ctxParams.flow_shift}};
    ...
    options.bool_options = {
        ...
        {"", "--chroma-disable-dit-mask", "disable dit mask for chroma", false, &params.ctxParams.chroma_use_dit_mask},
        {"", "--chroma-enable-t5-mask", "enable t5 mask for chroma", true, &params.ctxParams.chroma_use_t5_mask},
        {"", "--increase-ref-index", "automatically increase the indices of references images based on the order they are listed (starting with 1).", true, &params.lastRequest.increase_ref_index},
        {"", "--disable-auto-resize-ref-image", "disable auto resize of ref images", false, &params.lastRequest.auto_resize_ref_image},
    ...
    auto on_rng_arg = [&](int argc, const char** argv, int index) {
        ...
        params.ctxParams.rng_type = str_to_rng_type(arg);
        if (params.ctxParams.rng_type == RNG_TYPE_COUNT) {
    ...
    auto on_sampler_rng_arg = [&](int argc, const char** argv, int index) {
        ...
        const char* arg         = argv[index];
        params.ctxParams.sampler_rng_type = str_to_rng_type(arg);
        if (params.ctxParams.sampler_rng_type == RNG_TYPE_COUNT) {
    ...
    auto on_sample_method_arg = [&](int argc, const char** argv, int index) {
        ...
        const char* arg                                = argv[index];
        params.lastRequest.sample_params.sample_method = str_to_sample_method(arg);
        if (params.lastRequest.sample_params.sample_method == SAMPLE_METHOD_COUNT) {
    ...
    auto on_prediction_arg = [&](int argc, const char** argv, int index) {
        ...
        const char* arg             = argv[index];
        params.ctxParams.prediction = str_to_prediction(arg);
        if (params.ctxParams.prediction == PREDICTION_COUNT) {
    ...
    auto on_lora_apply_mode_arg = [&](int argc, const char** argv, int index) {
        ...
        const char* arg                    = argv[index];
        params.ctxParams.lora_apply_mode = str_to_lora_apply_mode(arg);
        if (params.ctxParams.lora_apply_mode == LORA_APPLY_MODE_COUNT) {
    ...
    auto on_type_arg = [&](int argc, const char** argv, int index) {
        ...
        const char* arg      = argv[index];
        params.ctxParams.wtype = str_to_sd_type(arg);
        if (params.ctxParams.wtype == SD_TYPE_COUNT) {
    ...
    options.manual_options = {
        ...
        {"", "--lora-apply-mode", "the way to apply LoRA, one of [auto, immediately, at_runtime], default is auto. In auto mode, if the model weights contain any quantized parameters, the at_runtime mode will be used; otherwise, immediately will be used.The immediately mode may have precision and compatibility issues with quantized parameters, but it usually offers faster inference speed and, in some cases, lower memory usage. The at_runtime mode, on the other hand, is exactly the opposite.", on_lora_apply_mode_arg},
        {"", "--scheduler", "denoiser sigma scheduler, one of [discrete, karras, exponential, ays, gits, smoothstep, sgm_uniform, simple, lcm], default: discrete", on_schedule_arg},
    ...
    auto task = [req, &sd_ctx, &params, &n_prompts, task_id]() {
        ...
        if (sd_ctx == NULL) {
            ...
            sd_ctx_params_t sd_ctx_params = {
                ...
                params.ctxParams.t5xxl_path.c_str(),
                (params.ctxParams.llm_path.empty() ? params.ctxParams.qwen2vl_path.c_str() : params.ctxParams.llm_path.c_str()),
                (params.ctxParams.llm_vision_path.empty() ? params.ctxParams.qwen2vl_vision_path.c_str() : params.ctxParams.llm_vision_path.c_str()),
                params.ctxParams.diffusion_model_path.c_str(),
...

        {
            ...
            sd_set_preview_callback((sd_preview_cb_t)step_callback, params.lastRequest.preview_method, params.lastRequest.preview_interval, !params.lastRequest.preview_noisy, params.lastRequest.preview_noisy, (void*)&params);
